...
void FTLDumpHits::analyze(edm::Event const& event, edm::EventSetup const& setup)
{
    ...
    int idx=0;
    for(unsigned iTrack = 0; iTrack < extTracks.size(); ++iTrack)
    {
        auto track = tracks.at(iTrack);
        auto extTrack = extTracks.at(iTrack);
        // skip neutrals
        ...
        for(auto& genPart : genParticles)
	{
        ...
        //---matching to trackingParticles (aka SimTracks)
        // match with gen particles
        DRMin = 999999;
        int simPdgId = 0;
        float simEta = -999.;
        float simPhi = -999.;
        float simPt = -999.;
        reco::TrackBaseRef trackRef(tracksHandle_, iTrack);
        if(trkRecoToSimMap.find(trackRef) != trkRecoToSimMap.end())
        {
            simPdgId = trkRecoToSimMap[trackRef].begin()->first->pdgId();
            simEta = trkRecoToSimMap[trackRef].begin()->first->eta();
            simPhi = trkRecoToSimMap[trackRef].begin()->first->phi();
            simPt = trkRecoToSimMap[trackRef].begin()->first->pt();
        }
        ...
        else
	{
        ...
        outTree_.extTrack_pt -> push_back(extTrack.pt());
        outTree_.extTrack_eta -> push_back(extTrack.eta());
        outTree_.extTrack_phi -> push_back(extTrack.phi());
        outTree_.extTrack_x -> push_back(extTrack.vx());
        outTree_.extTrack_y -> push_back(extTrack.vy());
        outTree_.extTrack_z -> push_back(extTrack.vz());
        outTree_.extTrack_t -> push_back(extTrack.t0());
        ...
        outTree_.track_hasMTD -> push_back(extTrack.isTimeOk());
        ...
        outTree_.track_mcMatch_genDR -> push_back(DRMin);
        outTree_.track_mcMatch_simPdgId -> push_back(simPdgId);        
        outTree_.track_mcMatch_simPt -> push_back(simPt);
        outTree_.track_mcMatch_simEta -> push_back(simEta);
        outTree_.track_mcMatch_simPhi -> push_back(simPhi);
        outTree_.track_mcMatch_simDR -> push_back(deltaR(extTracks.at(iTrack).eta(), extTracks.at(iTrack).phi(),
                                                         simEta, simPhi));
        ...
        if( !inETL )
	{
        ...
        ///ETL
        {
            ...
            double bestClusterDr=9999.;
            for(unsigned int iC=0; iC<outTree_.matchedClusters_track_DR->at(idx).size(); ++iC)
            {
                if(outTree_.matchedClusters_track_DR->at(idx)[iC] < bestClusterDr)
                {
                    bestClusterDr = outTree_.matchedClusters_track_DR->at(idx)[iC];
                    outTree_.matchedClusters_best->insert_or_assign(idx, iC);
                }
            }
    ...
    //---clear
    outTree_.matchedClusters_best->clear();
}
