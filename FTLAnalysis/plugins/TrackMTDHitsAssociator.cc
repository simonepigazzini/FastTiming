...
class TrackMTDHitsAssociatorT : public edm::stream::EDProducer<> {  
public:
    typedef typename TrackCollection:: value_type TrackType;
    typedef edm::View<TrackType> InputCollection;
  
    TrackMTDHitsAssociatorT(const ParameterSet& pset); 
    ...
    std::vector<pair<TransientTrackingRecHit::ConstRecHitPointer, double> > tryBTLLayers(const TrackType&, 
                                                                                         const MTDTrackingDetSetVector&,
                                                                                         const MTDDetLayerGeometry*, 
                                                                                         const MagneticField* field,
                                                                                         const Propagator* prop) const;

    std::vector<pair<TransientTrackingRecHit::ConstRecHitPointer, double> > tryETLLayers(const TrackType&,
                                                                                         const MTDTrackingDetSetVector&,
                                                                                         const MTDDetLayerGeometry*, 
                                                                                         const MagneticField* field,
                                                                                         const Propagator* prop) const;
    ...
    checkRecHitsOrdering(TransientTrackingRecHit::ConstRecHitContainer const & recHits) const {
        ...
        LogDebug("TrackMTDHitsAssociator") << "Impossible to determine the rechits order" <<endl;
...
private:
    ...
    const bool updateTraj_, updatePattern_;
...
template<class TrackCollection>  
TrackMTDHitsAssociatorT<TrackCollection>::TrackMTDHitsAssociatorT(const ParameterSet& iConfig) :
...
template<class TrackCollection>
void TrackMTDHitsAssociatorT<TrackCollection>::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
...
template<class TrackCollection>
template <class H, class T>
void TrackMTDHitsAssociatorT<TrackCollection>::fillValueMap(edm::Event& iEvent, const H& handle, const std::vector<T>& vec, const std::string& name) const {
...
template<class TrackCollection>
void TrackMTDHitsAssociatorT<TrackCollection>::produce( edm::Event& ev,
    ...
    for( const auto& track : tracks ) { 
        ...
        std::vector<pair<TransientTrackingRecHit::ConstRecHitPointer, double> > mtdthits;
...
namespace {    
    ...
    void find_hits_in_dets(const MTDTrackingDetSetVector& hits, const DetLayer* layer,
                           ...
                           std::vector<pair<TransientTrackingRecHit::ConstRecHitPointer, double> >& output) {
                ...
                for( const auto& detWithState : compDets ) {	
                    ...
                    for( auto detitr = range.first; detitr != range.second; ++detitr ) {
                        for( auto itr = detitr->begin(); itr != detitr->end(); ++itr ) {
                            ...
                            if( est.first ) { // just take the best chi2
                                if(est.second < best_chi2)
                                {
                                    output.insert(output.begin(), make_pair(hitbuilder.build(&(*itr)), est.second));
                                    best_chi2 = est.second;
                                }
                                else
                                    output.push_back(make_pair(hitbuilder.build(&(*itr)), est.second));
                            }	     
                        }
                    }
                }
...
template<class TrackCollection>
std::vector<pair<TransientTrackingRecHit::ConstRecHitPointer, double> >
TrackMTDHitsAssociatorT<TrackCollection>::tryBTLLayers(const TrackType& track,
...
template<class TrackCollection>
std::vector<pair<TransientTrackingRecHit::ConstRecHitPointer, double> >
TrackMTDHitsAssociatorT<TrackCollection>::tryETLLayers(const TrackType& track, 
                                                       const MTDTrackingDetSetVector& hits,
                                                       const MTDDetLayerGeometry* geo,
                                                       const MagneticField* field,
                                                       const Propagator* prop) const {
...
template<class TrackCollection>
reco::Track TrackMTDHitsAssociatorT<TrackCollection>::buildTrack(const reco::Track& orig,
...
typedef TrackMTDHitsAssociatorT<reco::TrackCollection> TrackMTDHitsAssociator;

DEFINE_FWK_MODULE(TrackMTDHitsAssociator);
