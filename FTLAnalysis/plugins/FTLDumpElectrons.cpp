...
void FTLDumpElectrons<ElectronCollectionT>::analyze(edm::Event const& event, edm::EventSetup const& setup)
{
    ...
    if(readFTLrecHits_)
    {
        ...
        setup.get<FastTimeGeometryRecord>().get("FastTimeBarrel", ftlGeoHandle);
    ...
    for(auto& ele : electrons)
    {
        ...
        {
          std::shared_ptr<const CaloCellGeometry> cellGeometry = ecalBarrelGeometry->getGeometry(EBDetId(ecal_hit.first));
